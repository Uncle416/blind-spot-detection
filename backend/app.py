...
current_data = {
    ...
    'obstacle_type': None,
    'system_status': None,
    'door_status': {'lock_status': car_locked, 'door_status': door_status}
}
...
def read_write_door_json():
    global current_data, car_locked, door_status
    while True:
        try:
            with lock:    
                with open(door_file_path, 'r') as f:
                    data = json.load(f)
                    current_data['door_status'] = data
...
def calculate_system_status():
    distance = current_data.get('ultra_sonic_distance')
...
def gen(picam2):
    try:
        while True:
            ...
            # Update the global current_data with the detected objects
            if detected_objects:
                current_data['obstacle_type'] = detected_objects[0]  # Use the first detected object
            else:
                current_data['obstacle_type'] = "None"
...
def update_obstacle_type():
    ...
    calculate_system_status()
    return jsonify({'message': 'Obstacle type updated', 'current_data': current_data})
...
def lock_car():
    ...
    return jsonify({'message': 'Cannot lock car', 'current_data': current_data}), 403
...
def unlock_car():
    ...
    car_locked = False
    current_data['door_status'] = {'lock_status': car_locked, 'door_status': door_status}
    return jsonify({'message': 'Car unlocked', 'current_data': current_data})
