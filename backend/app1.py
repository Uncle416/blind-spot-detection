...
# model = YOLOv5("/home/pi/DL-PART/DLP/yolov5s.pt")  # 闁瀚ㄥΟ鈥崇€?
# Function to handle YOLO model prediction
# def yolo_prediction(cap):
#     global obstacle_type
#     while True:
#         ret, frame = cap.read()
#         if not ret:
#             app.logger.warning("Failed to capture frame.")
#             continue
...
def video_feed():
    ...
    # Thread(target=yolo_prediction, args=(cap,)).start()
    return Response(video_feed_generator(cap),
...
if __name__ == '__main__':
    # Start the threads to read serial data periodically
